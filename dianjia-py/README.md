...
这是“店家”个人知识库的本地操作程序：负责导入对话、生成日报、提取并归档长期记忆、检索和 RAG 问答。知识库 Markdown 位于上级目录 [dianjia](../README.md)，默认 SQLite 索引位于本项目的 `data/dianjia.db`。

本 README 说明如何运行程序；目录规范、Source/Candidate/Decision 数据格式、归档规则和完整治理流程以上级目录的 [dianjia/README.md](../README.md) 为唯一准则。使用其他 AI 协助归档时，也应先让它阅读该文件。

## 快速开始

```bash
cd /Users/zhangshichao/Documents/Workspace/dianjia/dianjia-py
python main.py init
python main.py web
```
...
AI_MODEL=gpt-5
# 可选：chat_completions（默认）或 codex_cli
AI_PROVIDER=chat_completions
# AI_PROVIDER=codex_cli 时使用官方 Codex 客户端认证
# AI_CODEX_BIN=/Applications/ChatGPT.app/Contents/Resources/codex
```
...
`AI_PROVIDER=codex_cli` 时，程序改为调用本机官方 Codex CLI 的 Responses 通道；认证由 Codex 客户端处理，`AI_API_KEY` 和 `AI_BASE_URL` 不参与该通道。`AI_MODEL` 仍用于选择模型，`AI_CODEX_BIN` 可指定 Codex 可执行文件路径。未设置 `AI_PROVIDER` 时保持原有 Chat Completions 行为。
...
1. 先阅读 [dianjia/README.md](../README.md)，遵守 Source、Candidate、Decision 和 Markdown frontmatter 规范。
...
```text
程序：/Users/zhangshichao/Documents/Workspace/dianjia/dianjia-py
知识库：/Users/zhangshichao/Documents/Workspace/dianjia
索引：/Users/zhangshichao/Documents/Workspace/dianjia/dianjia-py/data/dianjia.db
```
...
完整数据流、数据字段和归档治理规则见 [dianjia/README.md](../README.md)。
